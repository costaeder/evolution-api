...
const setBucketPolicy = async () => {
  ...
  try {
    ...
    logger.info(`[S3 Service] Bucket policy applied to ${bucketName}`);
  } catch (error: any) {
    if (error?.code === 'NotImplemented') {
      logger.warn(`[S3 Service] setBucketPolicy not supported for ${bucketName}. Skipping.`);
    } else {
      logger.error(['[S3 Service] Failed to apply bucket policy', error]);
      throw error;
    }
  }
};
